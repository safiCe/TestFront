    ...
    return (
        <Grid container alignItems="center" justifyContent="center" spacing={3} rowSpacing={3}>
            ...
            <Grid item xs={4}>
                <Button onClick={() => {
                    if (selectedProduct) {
                        let newProduct = newOffer.products.find((product) => product._id== selectedProduct);
                        console.log(newProduct);
                        if (!newProduct) dispatch(setNewOffer({ ...newOffer, products: [...newOffer.products, { _id: selectedProduct }] }));
